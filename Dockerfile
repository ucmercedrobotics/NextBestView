...
RUN apt-get update && apt-get install -y git wget python3-pip vim net-tools netcat \
    ...
    # ros-humble-kortex-bringup ros-humble-ign-ros2-control
...
ENV KORTEX_WORKING_SHA=17919554b01662d81cbe6a747d3817eb1bcc03d1
...
RUN cd $KORTEX_WS && \
    ...
    cd src/ros2_kortex && git reset --hard $KORTEX_WORKING_SHA && \
    sed -i '/<exec_depend>.*gazebo_ros2_control.*<\/exec_depend>/d' kortex_bringup/package.xml && \
    cd ../.. && \
